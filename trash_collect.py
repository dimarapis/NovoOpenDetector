...
class TrashCollect():
    def __init__(self, argv):
        ...
        #print(image.shape)
        #print(img.shape)
        self.spot_detection = NovoOpenDetector(model=MODEL, classes = CLASSES, box_threshold=BOX_TRESHOLD, text_threshold=TEXT_TRESHOLD)
    ...
    def move_to_aruco(self, ID, marker_size, dist_offset=1.0, side_offset=0.0, global_orientation=False, update_global=False, cutoff_time=5):
        ...
        if aruco[0] == -1:
            ...
            while not id_found:
                ...
                if aruco[0] == -1:
                    iterations += 1
                    cv2.imwrite("failed/FailToFind{}_FrontLeft_iter{}.png".format(ID, iterations), self.front_left_buffer)
                    cv2.imwrite("failed/FailToFind{}_FrontRight_iter{}.png".format(ID, iterations), self.front_right_buffer)
                    cv2.imwrite("failed/FailToFind{}_Left_iter{}.png".format(ID, iterations), self.left_buffer)
                    cv2.imwrite("failed/FailToFind{}_Right_iter{}.png".format(ID, iterations), self.right_buffer)
                    cv2.imwrite("failed/FailToFind{}_Rear_iter{}.png".format(ID, iterations), self.rear_buffer)
                else:
                    id_found = True
    ...
    def arm_object_grasp_DTAI(self, method):
        ...
        if method == 'click':
            ...
            result_image, object, confidence, centroid =  self.spot_detection.detect(img)
            if object is not None:
                ...
                try: 
                    battery_state = self.robot_state_client.get_robot_state().battery_state
                    print(battery_state)
                    #battery_voltage = battery_state.voltage
                    #battery_charge_percentage = battery_state.relative_state_of_charge
                    #print(f"Battery Voltage: {battery_voltage} V")
                    #print(f"Battery Charge Percentage: {battery_charge_percentage} %")
                except:
                    pass
                print(40*'*')
            else: 
            ...
            #cv2.imshow('Returned image', result_image)
            #cv2.waitKey(0)


            if object is not None: # If banana
                self.g_image_click = centroid
                ...
                while True:
                    ...
                    if response.current_state == manipulation_api_pb2.MANIP_STATE_GRASP_SUCCEEDED:
                        ...
                        if gripper_pose > 10.0:
                            return 1
                        else:
                            print('returned 0 at gripper pose')
                            self.arm_gaze_pose()
                            return 0
                    
                    elif response.current_state == manipulation_api_pb2.MANIP_STATE_GRASP_FAILED:
                        self.arm_gaze_pose()
                        print('return 0 at state level')
                        return 0
    ...
    def gaze_on_ground(self):
        ...
        while True:
            for angle in body_rotations:
                ...
                ret_val = self.arm_object_grasp_DTAI('detection')
                
                if self.object_type == 'banana': # If banana
                    ID = 1
                    s_offset1 = 0.7
                    s_offset2 = 0.5
                    arm_offset = -0.1
                ...
                elif self.object_type == 'bottle': # If bottle
                    ID = 2
                    s_offset1 = -0.7
                    s_offset2 = -0.5
                    arm_offset = 0.1
                ...
                if ret_val == 1:
                    ...
                    iterations = 1
                    while ret_val == 0:
                        print(f'picking iterations {iterations}')
                        if iterations > 2: # Allow Spot to try a grasp 3 times before giving up
...
#####################################################################
'''
1) After looking left, middle, and rigt! (FINE)

  File "/home/dim/last_spot/tras_collect_2.py", line 739, in main
    ID, s_offset1, s_offset2, arm_offset, rot_val = self.gaze_on_ground()
TypeError: cannot unpack non-iterable NoneType object

5) change rotation of bins 

6) Grasp time exceeded 
